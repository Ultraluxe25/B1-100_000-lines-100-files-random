...
def show_quantity_of_pattern_match(pattern='abc') -> tuple:
    """
    Function for building 1 big csv file from 100 small (10M lines)
    ...
    pattern_match = 0
    lines_moved = 0
    with open('second_stage.csv', 'w', encoding='UTF-8') as big_file:
        for file in range(1, 101):
            with open(f'{file}.csv', 'r', encoding='UTF-8') as current_file:
